...
// ==================== 反循环启发式常量 ====================
static constexpr double TURN_COST = 0.1;             // 转弯代价（避免不必要的转弯）
static constexpr double OPENNESS_DRIFT_BONUS = 0.15; // 向更开阔空间移动的奖励
static constexpr double CENTER_DRIFT_BONUS = 0.05;   // 从中心向外漂移的轻微奖励
...
int survival_strategy(const State &s, int sy, int sx, stringstream &log_ss, const GridMask &M)
{
    ...
    for (int k = 0; k < 4; ++k)
    {
        ...
        // Anti-circling heuristics
        // 1) discourage unnecessary turning (keep going straight when safe)
        if (k != me.dir)
            safety_score *= (1.0 - TURN_COST);
        // 2) step toward more open space
        double op_now = calculate_openness(M, sy, sx);
        double op_next = calculate_openness(M, ny, nx);
        if (op_next > op_now)
            safety_score *= (1.0 + OPENNESS_DRIFT_BONUS);
        // 3) gentle outward drift from exact center when tie-like
        int cy = H / 2, cx = W / 2;
        if (abs(ny - cy) + abs(nx - cx) > abs(sy - cy) + abs(sx - cx))
            safety_score *= (1.0 + CENTER_DRIFT_BONUS);
