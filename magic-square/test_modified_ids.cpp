...
{
    printf("Testing corrected IDS...\n");
    for (int maxDepth = 0; maxDepth <= 5; maxDepth++)
    {
        printf("Depth %d: ", maxDepth);
        nodes.clear();
        std::stack<int> st;
    ...
    printf("No solution found within depth limit\n");
}
...
int main()
{
    ...
    printf("=== Testing Modified IDS Implementation ===\n");

    // 创建一个简单的测试用例
    MagicSquare test;
    test.createSolvedCube();
    test.rotate(0, true); // 一步操作，应该能用 0- 解决

    printf("Test case (1 step from solved):\n");
    test.print();
    printf("Is solved: %s\n\n", test.check() ? "Yes" : "No");
